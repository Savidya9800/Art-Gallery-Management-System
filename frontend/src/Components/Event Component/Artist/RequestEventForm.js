...
const RequestEventForm = () => {
  ...
  const packages = [
    { id: "package1", name: "Package 1", members: { min: 200, max: 500 }, budget: 10000, decoration: "Without", venue: "Community Center" },
    { id: "package2", name: "Package 2", members: { min: 200, max: 500 }, budget: 15000, decoration: "With", venue: "Community Center" },
    { id: "package3", name: "Package 3", members: { min: 501, max: 1000 }, budget: 20000, decoration: "Without", venue: "Conference Hall" },
    { id: "package4", name: "Package 4", members: { min: 501, max: 1000 }, budget: 25000, decoration: "With", venue: "Conference Hall" },
    { id: "package5", name: "Package 5", members: { min: 1001, max: 2000 }, budget: 30000, decoration: "Without", venue: "Local Park" },
    { id: "package6", name: "Package 6", members: { min: 1001, max: 2000 }, budget: 35000, decoration: "With", venue: "Local Park" },
    { id: "package7", name: "Package 7", members: { min: 1, max: 199 }, budget: 7000, decoration: "Without", venue: "Local Park" },
    { id: "package8", name: "Package 8", members: { min: 1, max: 199 }, budget: 9000, decoration: "With", venue: "Local Park" }
  ];
  const initialFormData = {
    name: "", artist: "", email: email || "", mobileNumber: "", memberCount: "", message: "", eventDate: null, status: "pending", budget: "", packageName: ""
  };
  const [formData, setFormData] = useState(initialFormData);
  const [isModalOpen, setIsModalOpen] = useState(false); // Force open
  const [userRequests, setUserRequests] = useState({ pending: [], accepted: [], rejected: [] });
  ...
  const handleMemberCountChange = (e) => {
    ...
    const matchedPackage = packages.find((pkg) => count >= pkg?.members?.min && count <= pkg?.members?.max);
    if (matchedPackage) {
  ...
  const filteredPackages = packages.filter(
    (pkg) =>
      formData.memberCount && formData.memberCount >= pkg.members.min && formData.memberCount <= pkg.members.max
  );
  return (
    <div>
      <NavigationBar />
  ...
  <div className="bg-white form-fields">
    {[
      ...
      {
        label: "Mobile Number", type: "text", name: "mobileNumber", value: formData.mobileNumber, maxLength: 10,
        onInput: (e) => (e.target.value = e.target.value.replace(/\D/g, "")),
      },
      {
        label: "Member Count", type: "number", name: "memberCount", value: formData.memberCount,min:"0",
        onKeyPress: (e) => (["-", "+", "e"].includes(e.key) && e.preventDefault()),
        onChange: handleMemberCountChange,
      },
      { label: "Budget", type: "number", name: "budget", value: formData.budget, disabled: !!formData.package },
      { label: "Selected Package", type: "text", name: "packageName", value: formData.packageName, readOnly: true },
      { label: "Message", type: "textarea", name: "message", value: formData.message },
    ].map(({ label, ...inputProps }, i) => (
      <div key={i} className="mb-4 bg-white">
        <label className="block font-semibold bg-white">{label}: </label>
...
{filteredPackages.length > 0 && (
  <div className="mt-6 package-selection">
    ...
    <p>Call - 011 2 232 456 <br />Email - eventadmin@gmail.com</p>
  </div>
)}
        ...
        <div className="p-4 border-l-4 border-yellow-600 rounded-lg request-card rejected" style={{ width: "1400px" }}>
          {editingRequestId === request._id ? (
            <div className="space-y-4 bg-red-50">
              <div>
                <label className="block font-semibold text-gray-700 bg-red-50">Mobile Number:</label>
                <input
                  type="text"
                  name="mobileNumber"
              ...
              <div>
                <label className="block font-semibold text-gray-700 bg-red-50">Message:</label>
                <textarea
                  name="message"
              ...
              <div className="flex space-x-4 bg-red-50">
                ...
                <Button onClick={() => setEditingRequestId(null)} variant="danger">Cancel</Button>
              </div>
            </div>
          ) : (
            <div className="w-full space-y-2">
              ...
              <p className="text-gray-600"><strong>Member Count:</strong> {request.memberCount}</p>
              <p className="text-gray-600"><strong>Mobile Number:</strong> {request.mobileNumber}</p>
              <p className="text-gray-600"><strong>Package:</strong> {request.packageName}</p>
              <p className="text-gray-600"><strong>Message:</strong> {request.message}</p>
              <p className="text-gray-600"><strong>Date:</strong> {request.eventDate ? new Date(request.eventDate).toLocaleString() : "No Date"}</p>
              <div className="flex space-x-4">
                <Button variant="primary" onClick={() => handleEditClick(request)}>Edit</Button>
                <Button variant="danger" onClick={() => deleteRequest(request._id)}>Delete</Button>
              </div>
            </div>
          )}
        </div>
               ...
               {userRequests.accepted.map((request) => (
                 <li key={request._id} className="mb-4">
                   <div className="p-4 border-l-4 border-green-600 rounded-lg request-card rejected" style={{ width: "1400px" }}>
                     ...
                     <p className="text-gray-600"><strong>Date:</strong> {request.eventDate ? new Date(request.eventDate).toLocaleString() : "No Date"}</p>
                     <div className="flex space-x-4">
                       <Button variant="info" onClick={() => handleChoosePackage(request._id)}>Generate PDF</Button>
                       <Button variant="dark" onClick={() => handleChoosePackage(request._id)}>Pay Now</Button>
                     </div>
                   </div>
                 </li>
               ))}
